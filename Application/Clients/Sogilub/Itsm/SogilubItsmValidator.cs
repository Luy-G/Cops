using FluentValidation;

namespace CyberOps.Application.Clients.Sogilub.Itsm;

public class SogilubItsmDtoValidator : AbstractValidator<SogilubItsmDto>
{
    public SogilubItsmDtoValidator()
    {
        RuleFor(x => x.Key).NotEmpty().WithErrorCode("TICKET_KEY_REQUIRED");

        RuleFor(x => x.IssueId)
            .Cascade(CascadeMode.Stop)
            .NotNull().WithErrorCode("ISSUE_ID_REQUIRED")
            .GreaterThan(0).WithErrorCode("ISSUE_ID_INVALID");

        RuleFor(x => x.Status).NotEmpty().WithErrorCode("STATUS_REQUIRED");
        RuleFor(x => x.IssueType).NotEmpty().WithErrorCode("ISSUE_TYPE_REQUIRED");
        RuleFor(x => x.Priority).NotEmpty().WithErrorCode("PRIORITY_REQUIRED");
        RuleFor(x => x.Summary).NotEmpty().WithErrorCode("SUMMARY_REQUIRED");
        RuleFor(x => x.Created).NotNull().WithErrorCode("CREATED_AT_REQUIRED");

        RuleFor(x => x.TimeSpentHours)
            .GreaterThanOrEqualTo(0)
            .When(x => x.TimeSpentHours.HasValue)
            .WithErrorCode("TIME_SPENT_HOURS_INVALID");

        RuleFor(x => x.FirstResponseDurationMs)
            .GreaterThanOrEqualTo(0)
            .When(x => x.FirstResponseDurationMs.HasValue)
            .WithErrorCode("FIRST_RESPONSE_DURATION_MS_INVALID");

        RuleFor(x => x.Resolved)
            .GreaterThan(x => x.Created!.Value)
            .When(x => x.Resolved.HasValue && x.Created.HasValue)
            .WithErrorCode("RESOLVED_AT_INVALID");

        RuleFor(x => x.Updated)
            .GreaterThanOrEqualTo(x => x.Created!.Value)
            .When(x => x.Updated.HasValue && x.Created.HasValue)
            .WithErrorCode("UPDATED_AT_INVALID");

        RuleFor(x => x.FirstResponseSlaBreached)
            .Must(x => x == null || x == "true" || x == "false")
            .WithErrorCode("FIRST_RESPONSE_SLA_BREACHED_INVALID");
    }
}
